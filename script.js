...
// ========== NOTIFICATION FUNCTION (FIX UNTUK IPHONE) ==========
function showNotification(message, isSuccess = true) {
    ...
    notif.style.webkitAnimation = 'slideDown 0.3s ease'; // Untuk Safari
    notif.style.border = '1px solid rgba(255,255,255,0.3)';
    
    document.body.appendChild(notif);
    
    // Force reflow untuk iPhone
    notif.offsetHeight;
    ...
    setTimeout(() => {
        ...
        notif.style.webkitTransition = 'opacity 0.5s'; // Untuk Safari
        setTimeout(() => {
            if (notif.parentNode) {
                notif.remove();
            }
...
// MOH login - FIX UNTUK IPHONE
document.addEventListener('DOMContentLoaded', function() {
    const mohLoginBtn = document.getElementById('mohLoginBtn');
    if (mohLoginBtn) {
        mohLoginBtn.addEventListener('click', function(e) {
            e.preventDefault();
            handleMohLogin();
        });
        
        // Untuk iPhone, tambah touch event
        mohLoginBtn.addEventListener('touchstart', function(e) {
            e.preventDefault();
            handleMohLogin();
        });
    }
...
// Toggle sidebar - FIX UNTUK IPHONE
if (menuBtn) {
...
function renderAduan(userType) {
    ...
    // Handle form submission untuk USER - FIX UNTUK IPHONE
    if (userType === 'user') {
        const hantarBtn = document.getElementById('hantarAduanBtn');
        if (hantarBtn) {
            // Click event untuk semua browser
            hantarBtn.addEventListener('click', async function(e) {
                e.preventDefault();
                await handleHantarAduan(e);
            });
            
            // Touch event khusus untuk iPhone
            hantarBtn.addEventListener('touchstart', async function(e) {
                e.preventDefault();
                await handleHantarAduan(e);
            });
        }
...
// Function untuk handle hantar aduan - FIX UNTUK IPHONE
async function handleHantarAduan(e) {
    e.preventDefault();
